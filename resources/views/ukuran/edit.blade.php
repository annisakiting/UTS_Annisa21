...
@section('content')
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit Ukuran') }}: {{ $measurement->measurement_name }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('ukuran.update', $measurement->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-4">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <input type="text" 
                               ...
                               value="{{ old('measurement_name', $measurement->measurement_name) }}"
                               required
                    ...
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div>
                                ...
                                <input type="number" step="any" name="lingkar_dada" value="{{ old('lingkar_dada', $measurement->details['lingkar_dada'] ?? '') }}" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            </div>
                            <div>
                                <label for="panjang_lengan" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Panjang Lengan (cm)</label>
                                <input type="number" step="any" name="panjang_lengan" value="{{ old('panjang_lengan', $measurement->details['panjang_lengan'] ?? '') }}" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            </div>
                            <div>
                                <label for="lebar_bahu" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Lebar Bahu (cm)</label>
                                <input type="number" step="any" name="lebar_bahu" value="{{ old('lebar_bahu', $measurement->details['lebar_bahu'] ?? '') }}" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            </div>
                            <div>
                                <label for="panjang_baju" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Panjang Baju (cm)</label>
                                <input type="number" step="any" name="panjang_baju" value="{{ old('panjang_baju', $measurement->details['panjang_baju'] ?? '') }}" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div>
                                ...
                                <input type="number" step="any" name="lingkar_pinggang" value="{{ old('lingkar_pinggang', $measurement->details['lingkar_pinggang'] ?? '') }}" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            </div>
                            <div>
                                <label for="lingkar_panggul" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Lingkar Panggul (cm)</label>
                                <input type="number" step="any" name="lingkar_panggul" value="{{ old('lingkar_panggul', $measurement->details['lingkar_panggul'] ?? '') }}" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            </div>
                            <div>
                                <label for="panjang_celana" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Panjang Celana/Rok (cm)</label>
                                <input type="number" step="any" name="panjang_celana" value="{{ old('panjang_celana', $measurement->details['panjang_celana'] ?? '') }}" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            </div>
                            <div>
                                <label for="lingkar_paha" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Lingkar Paha (cm)</label>
                                <input type="number" step="any" name="lingkar_paha" value="{{ old('lingkar_paha', $measurement->details['lingkar_paha'] ?? '') }}" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end mt-4">
                    <a href="{{ route('pelanggan.ukuran.index', $measurement->customer_id) }}" class="mr-4 text-gray-600 dark:text-gray-400 hover:text-gray-900">
                        Batal
                    </a>
